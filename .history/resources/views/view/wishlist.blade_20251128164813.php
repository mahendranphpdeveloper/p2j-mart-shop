                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                @foreach($wishlistItems as $item)
                                                    @php
                                                        // Always use the loaded product relationship
                                                        $product = $item->product;

                                                        // 1. Prefer the directly loaded productUnit (wishlist.productUnit), else fallback to product.productUnit as a last resort (for BC)
                                                        $productUnit = $item->productUnit ?: ($product?->productUnit);

                                                        // Prefer image from productUnit->productimage if present, else product->productImage, else fallback to placeholder
                                                        $unitImageObj = $productUnit?->productimage;
                                                        $mainImageObj = $product?->productImage;

                                                        $unitProductImage = $unitImageObj?->web_image_1 ?? null;
                                                        $mainProductImage = $mainImageObj?->web_image_1 ?? null;

                                                        // Compose image path (unit image if valid, else product, else placeholder)
                                                        if ($unitProductImage) {
                                                            $imagePath = asset('uploads/products/' . $unitProductImage);
                                                        } elseif ($mainProductImage) {
                                                            $imagePath = asset('uploads/products/' . $mainProductImage);
                                                        } else {
                                                        ...
                                                        // Set $productUnitId: always 0 if not set (handles null in log), to match current URL logic
                                                        $productUnitId = $productUnit?->product_unit_id ?? 0;

                                                        $productName = $product?->product_name ?? '';
                                                        $productSlug = $product?->slug ?? $productId;

                                                        // Always append 'unit' key, even if 0 (for consistency)
                                                        $productUrl = route('product.view', ['slug' => $productSlug, 'unit' => $productUnitId]);

                                                        // Prefer unit price if present, else empty
                                                        $unitPrice = $productUnit?->unit_price;
                                                    @endphp
                                                    <tr>
