...
class Orders extends Controller
{
    ...
    public function cancel(Request $request)
    {
        ...
        \Log::info('Cancel order requested via admin', ['order_id' => $orderId, 'request' => $request->all()]);
        ...
        try {
            $order = \App\Models\UserOrder::where('order_id', $orderId)->first();

            if (!$order) {
                \Log::warning('Order not found when attempting admin cancel', ['order_id' => $orderId]);
                return response()->json(['success' => false, 'message' => 'Order not found'], 404);
            }

            // "Release" reserved stock, if any (mimicking PaymentsController)
            $expiredReservations = \App\Models\StockReservation::where('status', 'reserved')
            ...
            \Log::info("Order #{$order->order_id} cancelled by admin");
            ...
            \Log::error('Error cancelling order via admin: ' . $e->getMessage(), [
                'order_id' => $orderId,
