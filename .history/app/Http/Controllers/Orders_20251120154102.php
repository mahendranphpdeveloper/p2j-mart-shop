...
class Orders extends Controller
{
    ...
    public function processReturnAction(Request $request)
    {
        ...
        // Only allow update if return_status is 'requested'
        if (strtolower($order->return_status) !== 'requested') {
            return response()->json([
                'success' => false,
        ...
        // TODO: Optionally trigger notification/event here

        return response()->json([
            'success' => true,
            'message' => 'Return request ' . ($action === 'approved' ? 'approved' : 'rejected') . ' successfully.',
            'order_id' => $order->order_id,
    ...
    public function cancel(Request $request)
    {
        try {
            ...
            $order->update([
                'order_status' => 'cancelled', // Changed from 'status' to 'order_status' to match your DB
                'cancelled_at' => now()
            ]);

            \Log::info("Order #{$order->order_id} cancelled");
