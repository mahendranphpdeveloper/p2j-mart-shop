...
class Orders extends Controller
{
    ...
    /**
     * Admin cancels any order by ID (via AJAX or API)
     * Route: PUT /admin/orders/cancel
    ...
    public function cancel(Request $request)
    {
        ...
        try {
            ...
            if (in_array($order->order_status, ['cancelled', 'delivered'])) {
                ...
                return response()->json([
                    'success' => false,
                    'message' => 'Order cannot be cancelled (already delivered or cancelled)'
                ], 400);
            }
            ...
            // Optionally increment product quantity if not already handled
            // (Admin panel may want original stock restored as well)
            ...
            // Mark the order as cancelled with a reason
            $order->order_status = 'cancelled';
            $order->cancel_order_reason = $cancelReason;
            $order->cancelled_at = now();
            $order->save();

            // Optionally: send email to user (suppress errors)
            try {
                $user = $order->user;
                if ($user && filter_var($user->email, FILTER_VALIDATE_EMAIL)) {
                    Mail::to($user->email)->send(new OrderReject($order));
                }
            } catch (\Exception $mailEx) {
                \Log::error("Failed to send cancellation mail (admin cancel): " . $mailEx->getMessage());
            }
            ...
            return response()->json([
                'success' => true,
                'message' => 'Order cancelled successfully and stock released (if reserved).'
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error('Admin order cancellation failed', ['error' => $e->getMessage(), 'order_id' => $orderId]);
