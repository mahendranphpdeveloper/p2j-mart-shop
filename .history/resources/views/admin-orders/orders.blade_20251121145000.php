                ...
                <div class="card-body mt-2">
                    ...
                    <script>
                        $(document).ready(function() {
                            // Ensure status filter buttons filter correct tab's table based on visible tab
                            $('#order-status-filter').on('click', '.status-filter-btn', function() {
                                $('#order-status-filter .status-filter-btn').removeClass('active');
                                $(this).addClass('active');
                                var selectedStatus = $(this).data('status') || '';
                                
                                // Find active tab table
                                var $activeTab = $('#ordersTabsContent .tab-pane.active');
                                var $table = $activeTab.find('.datatables-order');
                                // Change data param for DataTables and reload
                                $table.DataTable().settings()[0].ajax.data = function(d) {
                                    d.order_type = $table.data('order-type');
                                    d.order_status = selectedStatus;
                                };
                                $table.DataTable().ajax.reload();
                            });
                            // On tab switch, reset status filter to All
                            $('a[data-bs-toggle="tab"][href^="#"]').on('shown.bs.tab', function () {
                                $('#order-status-filter .status-filter-btn').removeClass('active');
                                $('#order-status-filter .status-filter-btn[data-status=""]').addClass('active');
