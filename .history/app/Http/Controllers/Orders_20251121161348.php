...
class Orders extends Controller
{
    ...
    public function index()
    {
        ...
        $orderCounts = DB::table('user_orders')
            ...
            ->selectRaw('SUM(CASE WHEN order_status = "returned" AND return_status = "requested" THEN 1 ELSE 0 END) as returned_requested_count')
            ->selectRaw('SUM(CASE WHEN order_status = "returned" AND return_status = "approved" THEN 1 ELSE 0 END) as returned_approved_count')
            ->selectRaw('SUM(CASE WHEN order_status = "returned" AND return_status = "cancelled" THEN 1 ELSE 0 END) as returned_rejected_count')
            ->first();
        ...
        return view('admin-orders.orders', [
            ...
            'admin_rejected' => $orderCounts->cancelled_count + $orderCounts->returned_approved_count,
            'orders_returned' => $orderCounts->returned_count,
            'orders_returned_requested' => $orderCounts->returned_requested_count,
            'orders_returned_approved' => $orderCounts->returned_approved_count,
            'orders_returned_rejected' => $orderCounts->returned_rejected_count
        ]);
    }
    ...
    public function get(Request $request)
    {
        ...
        // Apply order_status filter if present in request (case-insensitive match)
        $order_status = $request->input('order_status');
        if (!empty($order_status)) {
            $query->whereRaw('LOWER(order_status) = ?', [strtolower($order_status)]);
        }
