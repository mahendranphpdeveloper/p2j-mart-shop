...
$(document).ready(function() {
  // Open menu
  $('.gi-site-menu-icon').on("click", function (e) {
    e.preventDefault();
    e.stopPropagation();
  ...
  // Close menu
  $('.gi-mobile-menu-overlay, .gi-close-menu').on("click", function (e) {
    e.preventDefault();
    closeMenu();
  });

  // Prevent menu from closing when clicking inside
  $('.gi-mobile-menu').on('click', function(e) {
    e.stopPropagation();
  });

  // Close menu when clicking outside
  $(document).on('click', function(e) {
    if (!$(e.target).closest('.gi-mobile-menu').length) {
      closeMenu();
    }
  });
  ...
  // Debug: Log when menu is opened or closed
  $('.gi-mobile-menu').on('addClass removeClass', function(e, className) {
    if (className === 'gi-menu-open') {
      console.log(e.type === 'addClass' ? 'Menu opened' : 'Menu closed');
    }
  });
});
...
QtyPlusMinus.find(".dec, .inc").remove(); // Remove existing buttons
