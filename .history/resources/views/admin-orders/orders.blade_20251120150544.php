                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                ...
                                // Handle different return_status states and show a button if return_status is present
                                if (row.return_status && typeof row.return_status === 'string') {
                                    const returnStatus = row.return_status.toLowerCase();
                                    let buttonLabel = '';
                                    switch (returnStatus) {
                                        case 'requested':
                                            buttonLabel = 'Return Requested';
                                            break;
                                        case 'cancelled':
                                            buttonLabel = 'Return Cancelled';
                                            break;
                                        case 'approved':
                                            buttonLabel = 'Return Approved';
                                            break;
                                        case 'returned':
                                            buttonLabel = 'Returned';
                                            break;
                                        default:
                                            buttonLabel = 'Return Status';
                                            break;
                                    }
                                    ...
                                    return `
                                        <button type="button"
                                            class="btn btn-sm ${returnStatus === 'returned' ? 'btn-success' : (returnStatus === 'approved' ? 'btn-primary' : (returnStatus === 'requested' ? 'btn-warning' : 'btn-secondary'))}"
                                            onclick="window.showReturnDetailsModal('${reason}','${details}','${orderId}')"
                                            style="min-width:120px;font-weight:500;"
                                        >
                                            <i class="fas fa-undo-alt" style="margin-right:.5em"></i>${buttonLabel}
                                        </button>
                                    `;
                                } else {
