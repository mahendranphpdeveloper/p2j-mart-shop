...
@section('layoutContent')
 ...
 <style>
    .return-reason-glow {
        box-shadow: 0 0 0px #671db0;
    }
    .return-reason-glow-active,
    .return-reason-glow:hover {
        box-shadow: 0 0 16px 2px #36f2ab;
    }
    .return-reason-glow i {
        text-shadow: 0 0 7px #36f2ab, 0 0 2px #22ed87;
    }
    .return-reason-glow:not(.return-reason-glow-active):not(:hover) i {
        text-shadow: none;
    }
</style>
    <!-- CSRF for all AJAX -->
                        ...
                        { data: 'payment_status', name: 'method', render: function (data) { return `<span class="badge ${getPaymentStatusBadge(data)}">${data}</span>`; } },
                        {
                            ...
                            render: function (data, type, row) {
                                if (data && data.toLowerCase() === 'returned') {
                                    ...
                                    return `
                                            <span 
                                                class="badge ${getOrderStatusBadge(data)} return-reason-glow"
                                                style="cursor:pointer; position:relative; transition: box-shadow 0.3s, text-shadow 0.3s;"
                                                onclick="window.showReturnDetailsModal('${row.return_reason ?? ''}','${row.return_details ?? ''}', '${row.order_id}')"
                                                onmouseenter="this.classList.add('return-reason-glow-active')" 
                                                onmouseleave="this.classList.remove('return-reason-glow-active')"
                                            >
                                                <i class="bi bi-chat-dots-fill" style="color:#22ed87; font-size:1.3em; vertical-align:middle; transition: text-shadow 0.3s;"></i>
                                                ${data}
                                            </span>
