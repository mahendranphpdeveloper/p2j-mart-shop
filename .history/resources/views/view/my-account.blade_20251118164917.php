            ...
            <div class="row">
                ...
                <div class="col-lg-9 col-md-12 col-sm-12">
                    ...
                    <div class="account-content">
                        <!-- Success/Error Alert -->
                        <div class="alert alert-success alert-dismissible fade show" role="alert" style="display: none;" id="success-message">
                            <span></span>
                            <button type="button" class="close" data-dismiss="alert"><span>×</span></button>
                        </div>
                        <div class="alert alert-danger alert-dismissible fade show" role="alert" style="display: none;" id="error-message">
                            <span></span>
                            <button type="button" class="close" data-dismiss="alert"><span>×</span></button>
                        </div>
                            ...
                            <table class="orders-list account-page">
                                ...
                                <tbody id="ordersTable">
                                    @forelse($orders as $order)
                                        <tr class="order-item" data-status="{{ $order->order_status }}" data-order-id="{{ $order->order_id }}">
                                            <td>
                                                @if(isset($order->items_details) && count($order->items_details) > 0)
                                                    <div class="order-products-preview d-flex align-items-center">
                                                        @foreach($order->items_details as $item)
                                                            <img src="{{ $item['image'] }}" alt="{{ $item['product_name'] }}" class="order-img me-1" style="width:45px;height:45px;object-fit:cover;border-radius:5px;border:1px solid #eee;">
                                                            @break
                                                        @endforeach
                                                        @if(count($order->items_details) > 1)
                                                            <span class="ms-2 text-secondary" style="font-size:12px;">+{{ count($order->items_details)-1 }} more</span>
                                                        @endif
                                                    </div>
                                                    <div class="order-product-names mt-1">
                                                        <span style="font-size:12px;color:#888;">
                                                        @foreach($order->items_details as $idx=>$item)
                                                            {{ $item['product_name'] }}@if($item['quantity'] > 1) <span style="color:#aaa;">(x{{ $item['quantity'] }})</span>@endif@if(!$loop->last), @endif
                                                        @endforeach
                                                        </span>
                                                    </div>
                                                @else
                                            ...
                                            <td class="actions-cell">
                                                {{-- <a href="{{ route('product.view', ['id' => $order->product_id]) }}" class="view-btn" title="View Product">
                                                    <i class="fas fa-eye"></i>
                                                </a> --}}
                                                <a href="{{ route('order.invoice', $order->order_id) }}" target="_blank" class="download-btn" title="Download Invoice">
                                                    <i class="fas fa-file-download"></i>
                                                </a>
                                                @if(in_array($order->order_status, ['processing', 'shipped']) && $order->tracking_id)
                                                    <a href="#" class="track-btn" title="Track Order" data-bs-toggle="modal" data-bs-target="#trackOrderModal{{ $order->order_id }}">
                                                        <i class="fas fa-map-marker-alt"></i>
                                                    </a>
                                                @endif
                                                @if($order->order_status == 'processing')
                                                    <a href="#" class="cancel-btn" title="Cancel Order" data-bs-toggle="modal" data-bs-target="#cancelOrderModal{{ $order->order_id }}">
                                                        <i class="fas fa-times-circle"></i>
                                                    </a>
                                                @endif
                                                @if($order->order_status == 'delivered')
                                                    <a href="#" class="return-btn" title="Return Order" data-bs-toggle="modal" data-bs-target="#returnOrderModal{{ $order->order_id }}">
                                        ...
                                        @if($order->tracking_id)
                                            <div class="modal fade" id="trackOrderModal{{ $order->order_id }}" tabindex="-1" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            ...
                                                            @if($order->shipped_at)
                                                                <p><strong>Shipped On:</strong> {{ $order->shipped_at->format('M d, Y') }}</p>
                                                            @endif
                                                            <a href="{{ $order->tracking_link }}" target="_blank" class="btn btn-primary mt-3">
                                        ...
                                        <div class="modal fade" id="cancelOrderModal{{ $order->order_id }}" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <form class="cancel-order-form" data-order-id="{{ $order->order_id }}">
                                                        @csrf
                                                        <div class="modal-header">
                                                            <h5 class="modal-title">Cancel Order #ORD-{{ $order->order_id }}</h5>
                                                            <button type="button" class="close" data-dismiss="modal"><span>×</span></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <textarea name="cancel_reason" class="form-control" rows="3" placeholder="Enter reason for cancellation..." required></textarea>
                                        ...
                                        <div class="modal fade" id="returnOrderModal{{ $order->order_id }}" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <form action="{{ route('order.return', $order->order_id) }}" method="POST">
                                                        @csrf
                                                        <div class="modal-header">
                                                            <h5 class="modal-title">Return Order #ORD-{{ $order->order_id }}</h5>
                                                            <button type="button" class="close" data-dismiss="modal"><span>×</span></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            ...
                                                            <textarea class="form-control" name="return_details" rows="3" placeholder="Additional details (optional)"></textarea>
                                                        </div>
                                                        <div class="modal-footer">
                                ...
                                <div class="d-flex justify-content-center mt-4">
                                    <nav aria-label="Page navigation">
                                        <ul class="pagination" style="flex-wrap: wrap; gap: 5px;">
                                            ...
                                            @else
                                                <li class="page-item">
                                                    <a class="page-link" href="{{ $orders->previousPageUrl() }}" style="border-radius: 0.25rem;" rel="prev">«</a>
                                                </li>
                                            @endif
                                            @foreach($orders->getUrlRange(1, $orders->lastPage()) as $page => $url)
                                                @if($page == $orders->currentPage())
                                                    <li class="page-item active" aria-current="page">
                                                        <span class="page-link" style="background-color: #0d6efd; border-color: #0d6efd; border-radius: 0.25rem;">{{ $page }}</span>
                                                    </li>
                                                @else
                                                    <li class="page-item">
                                                        <a class="page-link" href="{{ $url }}" style="border-radius: 0.25rem;">{{ $page }}</a>
                                                    </li>
                                                @endif
                                            @endforeach
                                            @if($orders->hasMorePages())
                                                <li class="page-item">
                                                    <a class="page-link" href="{{ $orders->nextPageUrl() }}" style="border-radius: 0.25rem;" rel="next">»</a>
                                                </li>
                                            @else
                                                <li class="page-item disabled" aria-disabled="true">
                                                    <span class="page-link" style="border-radius: 0.25rem;">»</span>
                        ...
                        <div id="addresses">
                            ...
                            <button class="btn btn-primary mb-3" data-toggle="modal" data-target="#addAddressModal">Add New Address</button>
                                ...
                                @foreach($addresses as $address)
                                    <div class="address-card" id="address-{{ $address->id }}">
                                        ...
                                        <div class="address-actions">
                                            <a href="#" class="address-btn edit-btn" data-toggle="modal" data-target="#editAddressModal{{ $address->id }}">Edit</a>
                                            <form method="POST" action="{{ route('address.delete', $address->id) }}" class="d-inline" style="display:inline;">
                                                @csrf
                                                @method('POST')
                        ...
                        <div id="wishlist">
                            <table class="table table-responsive account-wishlist">
                                ...
                                <tbody>
                                    <tr>
                                        <td class="product-thumbnail"><a href="#"><img src="assets/images/toy-1.jpg" alt="product1"></a></td>
                                        ...
                                        <td class="product-add-to-cart"><a href="#" class="btn btn-fill-out"><i class="fa-solid fa-cart-shopping"></i> Add to Cart</a></td>
                                        <td class="product-remove" data-title="Remove"><a href="#" class="remove-icon d-none d-md-inline"><i class="fa-solid fa-xmark"></i></a>
                                            <a href="#" class="remove-text d-inline d-md-none btn btn-danger btn-sm">Remove</a>
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="product-thumbnail"><a href="#"><img src="assets/images/toy-2.jpg" alt="product2"></a></td>
                                        <td class="product-name" data-title="Product"><a href="#">Lether Gray Tuxedo</a></td>
                                        <td class="product-price" data-title="Price">₹55.00</td>
                                        <td class="product-stock-status" data-title="Stock Status"><span class="badge rounded-pill text-bg-danger">No Stack</span></td>
                                        <td class="product-add-to-cart"><a href="#" class="btn btn-fill-out"><i class="fa-solid fa-cart-shopping"></i> Add to Cart</a></td>
                                        <td class="product-remove" data-title="Remove"><a href="#" class="remove-icon d-none d-md-inline"><i class="fa-solid fa-xmark"></i></a>
                                            <a href="#" class="remove-text d-inline d-md-none btn btn-danger btn-sm">Remove</a>
                                        </td>
                                    ...
                                    <tr>
                                        <td class="product-thumbnail"><a href="#"><img src="assets/images/toy-3.jpg" alt="product3"></a></td>
                                    ...
                                    <tr>
                                        <td class="product-thumbnail"><a href="#"><img src="assets/images/toy-4.jpg" alt="product3"></a></td>
                                        <td class="product-name" data-title="Product"><a href="#">woman full sliv dress</a></td>
                                        <td class="product-price" data-title="Price">₹68.00</td>
                                        <td class="product-stock-status" data-title="Stock Status"><span class="badge rounded-pill text-bg-success">In Stock</span></td>
                                        <td class="product-add-to-cart"><a href="#" class="btn btn-fill-out"><i class="fa-solid fa-cart-shopping"></i> Add to Cart</a></td>
                                    ...
                                    <tr>
                                        <td class="product-thumbnail"><a href="#"><img src="assets/images/toy-5.jpg" alt="product3"></a></td>
                                        <td class="product-name" data-title="Product"><a href="#">woman full sliv dress</a></td>
                                        <td class="product-price" data-title="Price">₹68.00</td>
                                        <td class="product-stock-status" data-title="Stock Status"><span class="badge rounded-pill text-bg-success">In Stock</span></td>
                                        <td class="product-add-to-cart"><a href="#" class="btn btn-fill-out"><i class="fa-solid fa-cart-shopping"></i> Add to Cart</a></td>
