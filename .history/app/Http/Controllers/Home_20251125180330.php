...
class Home extends Controller
{
    ...
    public function buyNowCheckout()
    {
        try {
            ...
            Log::info('BuyNow Checkout Controller received:', ['checkoutData' => $checkoutData]);

            // Fetch user's addresses
            $addresses = Address::where('user_id', $user->id)->get();
            Log::info('User addresses retrieved for BuyNow checkout', ['count' => $addresses->count()]);

            // Check for selected address in session
            $addressId = session()->get('selected_address_id');
            $selectedAddress = $addressId ? Address::find($addressId) : null;
            ...
            return view('products.checkout', [
                ...
                'custom_text' => $checkoutData['custom_text'] ?? null,
                'custom_image' => $checkoutData['custom_image'] ?? null,
                'selectedAttributes' => $checkoutData['selectedAttributes'] ?? [],
                'unitId' => $checkoutData['unitId'] ?? null,
                'user' => $user,
            ...
            Log::error('BuyNow checkout failed: ' . $e->getMessage());
            return redirect()->route('cart.view')
                ->with('error', 'An error occurred during buy now checkout');
        }
    }
    ...
    {
        // We mimic the Orders@cancel logic and adapt to the context of Home controller/user access.
        ...
        try {
            ...
            $expiredReservations = \App\Models\StockReservation::where('status', 'confirmed')
            ...
            foreach ($expiredReservations as $reservation) {
                ...
                if ($reservation->product_unit_id && $reservation->qty > 0) {
                    $productUnit = \App\Models\Productunit::where('product_unit_id', $reservation->product_unit_id)->first();
                    if ($productUnit) {
            ...
            // Actually mark order as cancelled (call "cancel" method, set reason from user)
            if (method_exists($order, 'cancel')) {
                $order->cancel($request->input('cancel_reason'));
            } else { // fail-safe for legacy models
                $order->order_status = 'cancelled';
                $order->cancel_order_reason = $request->input('cancel_reason');
    ...
    public function returnOrder(Request $request, UserOrder $order)
    {
        ...
        try {
            ...
            $order->update([
                ...
                'return_details' => $validated['return_details'] ?? null,
                'returned_at' => now()
            ]);
