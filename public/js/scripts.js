...
document.addEventListener("DOMContentLoaded", function () {
  const wishlistIcons = document.querySelectorAll(".wishlist-icon i"); // Select all icons
  ...
  wishlistIcons.forEach((icon) => {
      icon.addEventListener("click", function (event) {
          ...
          if (this.classList.contains("fa-regular")) {
              ...
              this.classList.add("fa-solid"); // Fill heart
          } else {
              this.classList.remove("fa-solid");
              this.classList.add("fa-regular"); // Outline heart
          }
      });
  });
});
...
$(document).ready(function () {
  ...
  $(document).click(function (event) {
      if (!$(event.target).closest("#filterSidebar, .filter-toggle").length) {
...
function rate(stars) {
    ...
    let starElements = document.querySelectorAll(".star-rating i");
...
function addReview() {
    ...
    newReview.innerHTML = `<div class='review-icon'>U</div><div class='review-text'><div class='review-author'>User</div><span class='stars'>${starsHTML}</span> - ${reviewText}</div>`;
...
document.addEventListener("DOMContentLoaded", function () {
  const menuLinks = document.querySelectorAll(".account-sidebar-menu a"); // Sidebar menu links
  const sections = document.querySelectorAll(".account-content > div"); // All sections
  ...
  function showSection(sectionId) {
      ...
      // Add 'active' to the clicked menu link
      document.querySelector(`.account-sidebar-menu a[data-section="${sectionId}"]`).classList.add("active");
  }
  ...
  // Show default section (Dashboard) on page load
  showSection("orders");
});
...
// Variables to track the current address being edited
...
function saveAddress() {
    ...
    if (!addressType || !name || !addressDetails || !phone) {
    ...
    if (currentAddressId) {
        ...
        const addressCard = document.getElementById(currentAddressId);
        addressCard.querySelector('.address-type').textContent = addressType;
        addressCard.querySelector('.address-name').textContent = name;
        addressCard.querySelector('.address-details').innerHTML = addressDetails.replace(/\n/g, '<br>');
        addressCard.querySelector('.address-details').nextElementSibling.textContent = `Phone: ${phone}`;
    } else {
        // Add new address
        const addressId = `address-${Date.now()}`; // Unique ID for the new address
        const addressCard = document.createElement('div');
        addressCard.className = 'address-card';
        addressCard.id = addressId;
        addressCard.innerHTML = `
            <span class="address-type">${addressType}</span>
            <div class="address-name">${name}</div>
            <div class="address-details">${addressDetails.replace(/\n/g, '<br>')}</div>
            <div>Phone: ${phone}</div>
            <div class="address-actions">
                <a href="#" class="address-btn edit-btn" onclick="editAddress('${addressId}')">Edit</a>
                <a href="#" class="address-btn delete-btn" onclick="deleteAddress('${addressId}')">Delete</a>
            </div>
        `;
        document.getElementById('addressesGrid').appendChild(addressCard);
    }
...
function editAddress(addressId) {
    const addressCard = document.getElementById(addressId);
    ...
    const addressDetails = addressCard.querySelector('.address-details').textContent;
    const phone = addressCard.querySelector('.address-details').nextElementSibling.textContent.replace('Phone: ', '');

    document.getElementById('addressType').value = addressType;
    document.getElementById('name').value = name;
    document.getElementById('addressDetails').value = addressDetails;
...
// account logout btn script

document.addEventListener("DOMContentLoaded", function () {
  document.getElementById("logoutBtn").addEventListener("click", function (event) {
      event.preventDefault(); 

      // Simulate user logout by clearing session/local storage
...
document.addEventListener("DOMContentLoaded", function () {
      ...
      orders.forEach(order => {
          if (filter === "all" || order.dataset.status === filter) {
              if (window.innerWidth <= 768) {
                  order.style.display = "block"; // Mobile layout (Card View)
              } else {
                  order.style.display = "table-row"; // Desktop layout (Table Row)
              }
          } else {
  ...
  // Re-apply the correct display property when window resizes
