...
<style>
   span.active {
                        ...
                        <div class="product_wrap">
                            <div class="product_img">
                                <a href="{{ route('product.show', $related->slug) }}">
                                    @if($related->images->first() && $related->images->first()->web_image_1)
                                        <img src="{{ asset('uploads/products/' . $related->images->first()->web_image_1) }}"
                                            alt="{{ $related->product_name }}" />
                                        <img class="product_hover_img"
                                            src="{{ asset('uploads/products/' . $related->images->first()->web_image_1) }}"
                                            alt="{{ $related->product_name }}" />
                                    @else
                                        <div class="text-center text-muted py-5" style="height: 250px; border: 1px solid #ccc;">
                                            No Image
                                        </div>
                                    @endif
                                </a>
                            ...
                            <div class="product_info">
                                <h6 class="product_title">
                                    <a href="{{ route('product.show', $related->slug) }}">{{ $related->product_name }}</a>
                                </h6>
                                @if($related->units->first())
                                        <div class="product_price">
                                            <span class="price">₹{{ $related->units->first()->unit_price }}</span>
                                            <del>₹{{ $related->units->first()->mrp_price }}</del>
                                            <div class="on_sale">
                                                <span>
                                                    {{
                                                        number_format(
                                                            100 * (($related->units->first()->mrp_price - $related->units->first()->unit_price) /
                                                                $related->units->first()->mrp_price)
                                                            ,
                                                            0
                                                        )
                                                            }}% Off
                                                ...
                                                <div class="cart_btn">
                                                    <button class="btn btn-fill-out btn-addtocart add-to-cart-btn" type="button"
                                                        data-product-id="{{ $product->product_id }}"
                                                        data-product-name="{{ $product->product_name }}"
                                                        data-price="{{ $unit->unit_price ?? 0 }}" data-quantity="1">
                                ...
                                <div class="pr_desc">
                                    <p>
                                        {{ Str::limit($related->short_description ?? 'No description available.', 100) }}
                                    </p>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        // Instead of handling buy now form submit via JS, in the Livewire component's backend you should now do:
        // $this->dispatch('buy-now', $checkoutData);
        // If you are handling "Buy Now" via JS and wish to fire a Livewire event from this Blade file, you can use:
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (buyNowForm) {
                buyNowForm.addEventListener('submit', function (e) {
                    ...
                    // Collect the checkout data from form inputs
                    // (Example: gather all form data as an object)
                    ...
                    if (window.Livewire && Livewire.dispatch) {
                        Livewire.dispatch('buy-now', checkoutData);
                    } else if (window.Livewire && Livewire.emit) {
                        // For backwards compatibility
                        Livewire.emit('buy-now', checkoutData);
                    } else {
                        alert('Livewire not found, cannot dispatch buy-now event.');
                    }
                });
            }
        });
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // (The custom validation logic for the form can remain, and dispatch will take care of triggering Livewire logic)
            if (buyNowForm) {
                buyNowForm.addEventListener('submit', function (e) {
