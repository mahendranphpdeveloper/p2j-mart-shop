...
Route::get('/product/{id}/{slug}', [Home::class, 'show'])
    ->name('product.viewd');
...
//  // For web interface (with auth protection)
// Route::get('/check-ccavenue-config', [PaymentsController::class, 'checkConfig'])
//     ->middleware('auth');

// // Or for API (if you need it)
// Route::get('/api/check-ccavenue-config', [PaymentsController::class, 'checkConfig'])
//     ->middleware('auth:api');

//     //test
//   Route::match(['get', 'post'], '/log-form-submission', function (Request $request) {
//     Log::debug('CCAvenue form submission', [
//         'order_id' => $request->input('order_id'),
//         'encRequestLength' => $request->input('encRequestLength'),
//         'accessCode' => $request->input('accessCode') ? 'set' : 'empty'
//     ]);
//     return response()->json(['status' => 'logged']);
// })->name('log.form.submission');

// Route::match(['get', 'post'], '/log-client-error', function (Request $request) {
...
// Route::match(['get', 'post'],'/debug-ccavenue', function(Request $request) {
//     \Log::debug('CCAvenue Debug', [
//         'encrypted_data' => substr($request->input('encRequest'), 0, 100) . '...',
...
//     // Get last 12 characters of APP_KEY as the valid token
//     $validToken = substr(env('APP_KEY'), -12); 
...
//     Route::get('/success/{order}', [OrderController::class, 'showSuccess'])->name('orders.success');
//     Route::post('/{order_id}/cancel', [OrderController::class, 'cancel'])->name('orders.cancel');
...
/////////////31-5 customize 
// Route::get('/customized-products', [ProductController::class, 'showCustomizedProducts'])->name('customized.products');
...
Route::get('/migrate-seed', function () {
    ...
    // Return a response to confirm the actions
    return "Migrations and seeds have been run successfully.";
});
